...
def _synthetic_raw(path: Path, n: int = 90) -> Path:
...
def test_pipeline_smoke(tmp_path: Path):
    ...
    if not raw.exists():
        raw = _synthetic_raw(tmp_path / "survey_raw.csv")
    ...
    assert {"ses_tertile", "otc_use_pct", "herbal_use_pct"}.issubset(prev.columns)
